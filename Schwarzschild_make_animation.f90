program main              !   KODAMA YUGA 201913527 BLACK HOLE CAMERA
    implicit none
    ...
    character(256) :: filename_cm, filename_it
  ...
  do angle_number = 0,180
    ...
    angle = dble(angle_number)*PI/180.0d0

    call make_file(angle_number,filename_cm,filename_it) ! 連番ファイル作成
!    open(102, file = filename_cm, status = 'unknown')
    open(103, file = filename_it, status = 'unknown')
    ...
    loop_b : do ! bが０になるまでのループ
      ...
      loop_r : do !ｒが事象の地平線にはいるまで計算
        ...
        if(b .GT. b_cut) then
        call Radiative_Transfer(u,theta,angle,intencity) !ステップごとに輻射輸送方程式で強度の更新
        end if
...
!  close(102)
  ...
  CONTAINS
    ...
    subroutine absorption(r,theta,phi,angle,kai) !------------------------------------------- 吸収係数の関数
      implicit none
      double precision, intent(in) :: r, theta, phi, angle
    ...
    subroutine Radiative_Transfer(u,theta,angle,intencity) ! -------------------------輻射輸送の時の積分
      implicit none
      integer :: i
      double precision, intent(in) :: u, theta, angle
      ...
      do i = 1,m
        ...
        call absorption(r,theta,phi,angle,kai) ! 吸収係数を求めている
    ...
    subroutine make_file(angle_number,filename_cm,filename_it)
      implicit none
      integer :: angle_number
      character(256) :: filename_cm, filename_it, &
                      & filehead_cm, filehead_it, &
                      & filetail, angle_index

      write(angle_index,'(i5.5)') angle_number
      write(angle_index,'(i5.5)') angle_number

      filehead_cm = "BlackHoleCamera_"
      filehead_it = "Intencity_elv_"

      filetail = ".dat"

      filename_cm = trim(filehead_cm)//trim(angle_index)//trim(filetail)
      filename_it = trim(filehead_it)//trim(angle_index)//trim(filetail)
    ...
    subroutine write_SphereSymmetry(b,intencity) !-----------------------------------ファイルの出力
      ...
      ! write(102,*) ''
      write(103,'(2f15.8)')  b, intencity(m/4)
      write(103,'(2f15.8)') -b, intencity(m/4)
      !---------------------------------------------------------!
